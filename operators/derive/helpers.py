...
def preprocess_services(select_fields=None, validate=False):
    return DF.Flow(
        ...
        DF.set_type('response_ids', transform=lambda _, row: row['responses_manual_ids'] if row['source'] in ('social-procurement', 'shil') else row['responses_ids'], resources=['services']),
