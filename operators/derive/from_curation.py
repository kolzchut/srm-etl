...
def copy_from_curation_base(curation_base, source_id):
    ...
    table_fields = {
        ...
        settings.AIRTABLE_BRANCH_TABLE: ['name', 'organization', 'operating_unit', 'address', 'address_details', 'location', 'description', 'phone_numbers', 'email_address', 'urls', 'situations'],
    ...
    for table in (settings.AIRTABLE_ORGANIZATION_TABLE, settings.AIRTABLE_BRANCH_TABLE, settings.AIRTABLE_SERVICE_TABLE):
        ...
        DF.Flow(
            ...
            DF.select_fields(table_fields[table] + ['decision', AIRTABLE_ID_FIELD] + ['status', 'id', 'source', 'fixes'] + extra_fields.get(table, [])),
