...
def upload_tileset(filename, tileset, name):
    ...
    s3_client.upload_file(filename, creds["bucket"], creds["key"])
    data = dict(tileset=tileset, url=creds["url"], name=name)
    ...
    while True:
        ...
        if status['complete']:
        status = requests.get(
            settings.MAPBOX_UPLOAD_STATUS + upload["id"], params=AUTH
        ).json()
            break
        time.sleep(10)
